...
import { _list } from '../../api/expressage'
  ...
  methods: {
      ...
      Promise.all(funcs)
        .then(res => {
          const totalCount = res.map(item => {
            let count = item.data.data ? item.data.data.DataCount : 0
            return count
          })
          const lists = res.map(item => {
            let list = item.data.data ? item.data.data.VBillList : []
            return list
          })
          this.set({
    ...
    loadMore(current) {
      ...
      _list({ UnionID, State: tabs[current].value, PageIndex, PageSize })
        .then(res => {
          const data = res.data.data.VBillList
